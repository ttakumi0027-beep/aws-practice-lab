...
| インスタンス名 | 配置サブネット | リージョン/アベイラビリティゾーン |　用途 |
|------|------|------|------|
...
- インスタンス（DB-server-1a）を下記のようにスケジュールを設定
...
- Elastic IP を利用することで、停止後も同じ IP を使えるようになった
- 起動テンプレートを利用するとインスタンス作成を自動化できる
- AMI は別リージョンにコピー可能で、災害復旧を想定したバックアップ戦略になる
- EBS ボリュームは増やせるが縮小できない制約がある
- スナップショットは AWS 側で管理される（自分の S3 バケットではない）
...
- SQL構文の確認
- MySQLを利用する予定だったが、Amazon Linux 2023は OpenSSL 3 系なのでOracle公式RPMはそのままでは依存関係エラーになったため、MariaDBを利用。→ Dockerなどを利用すればMySQLを使える。
